...
.dark .primary{background:#f7f8fa;color:#111827}
.secondary{
  background:var(--soft);
  color:var(--text);
  border:0;
...
<aside class="sidebar" id="sidebar">
  <div class="brand">📚 Study<span>Mate</span></div>
...
<div class="hero">
  <h1 id="heroTitle">Study smarter. Achieve more. 🚀</h1>
  <p>Everything you need to organise your study routine.</p>
</div>
...
<div class="row">
<h2>Your tasks</h2>
<select id="taskFilter" style="width:auto" onchange="renderTasks()">
...
<button class="secondary" onclick="requestNotifications()">Allow Notifications</button>
</div>
...
<button class="primary" onclick="setGoal()">Set Goal</button>

<h3 id="goalText">No goal set.</h3>
...
<span class="muted">Focus sessions</span>
</div>
...
<div class="progress-bg">
<div class="progress" id="statBar"></div>
</div>
<p id="statPercent">0%</p>
</div>
...
<h1>⭐ StudyMate Premium</h1>

<p class="muted">
Unlock the complete StudyMate experience.
</p>
...
<div class="feature">✅ Unlimited tasks</div>
<div class="feature">✅ Advanced statistics</div>
<div class="feature">✅ Unlimited reminders</div>
<div class="feature">✅ Extra themes</div>
<div class="feature">✅ Achievements</div>
<div class="feature">✅ Smart study planning</div>

<br>

<button class="primary" onclick="premiumDemo()">
Try Premium
</button>

<p class="muted">
Demo only — payments are not connected yet.
</p>
...
<h3 id="profileWelcome">Welcome to StudyMate! 👋</h3>
</div>
...
<div class="setting">
<strong>Theme</strong>
<p class="muted">Switch between light and dark mode.</p>
<button class="secondary" onclick="toggleTheme()">Toggle theme</button>
</div>

<div class="setting">
<strong>Notifications</strong>
<p class="muted">Allow StudyMate to request browser notifications.</p>
<button class="secondary" onclick="requestNotifications()">Enable notifications</button>
</div>

<div class="setting">
<strong>Reset data</strong>
<p class="muted">Delete tasks, goals, reminders and profile data.</p>
<button class="danger" onclick="resetAll()">Reset StudyMate</button>
</div>
...
function showPage(page){
 ...
 document.getElementById("topTitle").textContent=names[page];
 renderAll();
}
...
function addTask(){
 ...
 if(!subject||!text){
   alert("Please enter a subject and task.");
   return;
 }
...
function renderTasks(){
 ...
 <div class="task">
   ...
   <div class="task-info ${t.done?"done":""}">
     <strong>${safe(t.subject)}</strong>
     <span class="badge">${safe(t.category)}</span>
     <br>
     ${safe(t.text)}
     ${t.date?`<br><span class="muted">📅 ${safe(t.date)}</span>`:""}
   </div>
...
function renderDashboard(){
 ...
 const goalDone=Math.min(done,data.goal||0);
 document.getElementById("todayGoalLabel").textContent=
 data.goal?`${goalDone}/${data.goal}`:`${done}/0`;
 ...
 if(!future.length){
   upcoming.innerHTML='<div class="empty">No upcoming tasks.</div>';
 }else{
   upcoming.innerHTML=future.map(t=>`
    <div class="task">
      <div class="task-info">
       <strong>${safe(t.subject)}</strong>
       <span class="badge">${safe(t.category)}</span>
       <br>${safe(t.text)}
       <br><span class="muted">📅 ${safe(t.date)}</span>
      </div>
    </div>
   `).join("");
 }
...
function renderGoal(){
 ...
 const p=Math.min(100,Math.round(done/data.goal*100));
...
function renderCalendar(){
 ...
 ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].forEach(d=>{
  grid.innerHTML+=`<div class="day"><strong>${d}</strong></div>`;
 });
 ...
 for(let d=1;d<=last.getDate();d++){
  ...
  const has=data.tasks.some(t=>t.date===date);
  ...
  grid.innerHTML+=`
   <div class="day ${isToday?"today":""}">
    <strong>${d}</strong>
    ${has?'<br><span class="reminder-dot"></span>':""}
   </div>
  `;
 }

 const ct=document.getElementById("calendarTasks");

 const tasks=data.tasks.filter(t=>t.date);

 if(!tasks.length){
  ct.innerHTML='<div class="empty">No deadlines added.</div>';
 }else{
  ct.innerHTML=tasks
   .sort((a,b)=>a.date.localeCompare(b.date))
   .map(t=>`
...
function renderReminders(){
 ...
 if(!data.reminders.length){
  box.innerHTML='<div class="empty">No reminders yet.</div>';
  return;
 }
  ...
  <div class="reminder">
   <div class="row">
    <div>
     ...
     <br>
     <span class="muted">${formatDateTime(r.date)}</span>
    </div>
    <button class="danger" onclick="deleteReminder(${r.id})">🗑</button>
   </div>
  </div>
 `).join("");
}
...
function renderStats(){
 ...
 document.getElementById("statSessions").textContent=data.focusSessions;
 document.getElementById("statStreak").textContent=calculateStreak();
 document.getElementById("statBar").style.width=p+"%";
 document.getElementById("statPercent").textContent=p+"% completion rate";
}
...
function renderProfile(){
 ...
 data.name?`Welcome back, ${data.name}! 👋`:"Welcome to StudyMate! 👋";

 document.getElementById("heroTitle").textContent=
 data.name?`Welcome back, ${data.name}! 🚀`:"Study smarter. Achieve more. 🚀";
}
...
function premiumDemo(){
 alert(
  "⭐ Premium is ready for payment integration.\\n\\n" +
  "This demo does not process real payments yet."
 );
}
...
/* RESET */
function resetAll(){
 if(!confirm("Delete all StudyMate data?"))return;
